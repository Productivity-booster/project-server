...
const login = async (req, res) => {
  try {
    ...
    const newToken = jwt.sign({ username: username }, process.env.JWT_SECRET, {
      expiresIn: "2h",
    });
    ...
    res
      .status(200)
      .json({ message: "Login successful.", token: newToken, login: true });
  } catch (error) {
    res
      .status(400)
      .json({ message: "Error logging in, please try again!", login: false });
